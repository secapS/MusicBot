...
class Serializer(json.JSONEncoder):
    ...
    def _get_vars(cls, func):
        ...
        for name, param in params.items():
            ...
            if param.kind is param.POSITIONAL_OR_KEYWORD and \
...
class VoiceStateUpdate:
    ...
    def raw_change(self) -> dict:
        """ TODO """
        return objdiff(self.before.voice, self.after.voice,
    ...
    def changes(self):
        ...
        if any(s in rchange for s in ['mute', 'self_mute']):
            mute = rchange.get('mute', None) or rchange.get('self_mute')
            changes.append(self.Change.MUTE if mute[1] else self.Change.UNMUTE)

        if any(s in rchange for s in ['deaf', 'self_deaf']):
            deaf = rchange.get('deaf', None) or rchange.get('self_deaf')
            changes.append(
                self.Change.DEAFEN if deaf[1] else self.Change.UNDEAFEN)

        if 'is_afk' in rchange:
            changes.append(
                self.Change.MUTE if rchange['is_afk'][1] else
                self.Change.UNMUTE)
